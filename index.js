...
function listOfNames(array) {
  ...
  arrayFour.push(array[index].name)}
  return arrayFour;
};
...
function addArtist(array, name, years, genre, nationality,bio) {
  var copy = array.slice()
  copy.push ({name, years,  genre, nationality, bio})
  return copy;
}
...
function lotsOfArt(array) {
  ...
  for (var index = 0; index <= array.length-1; index++) {
    if(array[index].paintings > 100){
      arraySeven.push(array[index].name);
    }
  }
  return(arraySeven);
}
...
function artistInfo(array, nameString){
  var index = array.map(function(thisFunction) { 
...
function artistByCountry(array, country){
  const arrayEight = [];
  for (var index = 0; index <= array.length-1; index++) {
    if(array[index].nationality === country){
      arrayEight.push(array[index].name);
    }
  }
  return(arrayEight);
...
//note to self: do *NOT* forget to put VAR in front of INDEX next time! Spare yourself next time!
